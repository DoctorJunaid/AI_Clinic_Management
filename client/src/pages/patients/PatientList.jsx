...
const PatientList = () => {
    ...
    return (
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-4 shrink-0">
          <div>
            <h1 style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, fontSize: '1.5rem', letterSpacing: '-0.03em', color: 'var(--text-primary)', lineHeight: 1.2 }}>Patients Directory</h1>
            <p style={{ fontSize: '0.6875rem', color: 'var(--text-muted)', fontWeight: 500, marginTop: '2px', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{filteredPatients.length} EHR profiles</p>
          </div>
          <button 
            style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', padding: '0.4375rem 0.875rem', background: 'var(--primary)', color: '#ffffff', border: 'none', borderRadius: 'var(--r-md)', fontSize: '0.8125rem', fontWeight: 500, cursor: 'pointer', transition: 'background var(--t-fast) var(--ease-out)', letterSpacing: '-0.01em' }}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem', flexShrink: 0, background: 'var(--surface)', border: '1px solid var(--border)', padding: '0.625rem 0.75rem', borderRadius: 'var(--r-xl)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flex: 1, maxWidth: '320px', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', padding: '0.375rem 0.625rem', transition: 'border-color var(--t-fast)' }}>
            <Search size={14} style={{ color: 'var(--text-muted)', flexShrink: 0 }} />
            <input 
              style={{ width: '100%', background: 'transparent', border: 'none', fontSize: '0.8125rem', color: 'var(--text-primary)', outline: 'none', fontFamily: 'var(--font-sans)' }}
          ...
          <div style={{ display: 'flex', gap: '0.375rem', alignItems: 'center' }}>
            {[['all','All'],['highRisk','Risk Alerts'],['today','Today']].map(([val, label]) => (
              <button 
                key={val}
                ...
                style={{
                  ...
                  fontSize: '0.75rem',
                  fontWeight: activeFilter === val ? 600 : 400,
                  cursor: 'pointer',
                  border: activeFilter === val ? '1px solid var(--primary)' : '1px solid var(--border)',
                  background: activeFilter === val ? 'var(--primary)' : 'var(--surface)',
                  color: activeFilter === val ? '#ffffff' : 'var(--text-muted)',
        ...
        <div className="flex-1 overflow-y-auto pr-1">
          {filteredPatients.length === 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '4rem 2rem', background: 'var(--surface)', border: '1px dashed var(--border)', borderRadius: 'var(--r-xl)', textAlign: 'center', gap: '0.375rem' }}>
              <User size={28} style={{ color: 'var(--text-muted)', opacity: 0.4, marginBottom: '0.25rem' }} />
              <p style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-secondary)' }}>No patients found</p>
              <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Try a different search or filter</p>
            </div>
          ) : (
            <div style={{ overflowX: 'auto', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--r-xl)', boxShadow: 'var(--shadow-xs)' }}>
              <table className="patient-directory-table">
                <thead>
                  <tr>
                    ...
                    <th>Demographics</th>
                    <th>Primary Phone</th>
                    <th>Clinical Allergies</th>
                    <th>Registered Date</th>
                    <th className="text-right">EHR Actions</th>
                  </tr>
                </thead>
                  ...
                  {filteredPatients.map((p) => (
                    ...
                    >
                      <td>
                        <div className="flex items-center gap-4">
                          <div className="avatar-container">
                            <img src={getPatientAvatar(p)} alt={p.name} className="avatar-img" />
                            {p.allergies?.length > 0 && (
                              <span className="absolute -top-0.5 -right-0.5 w-3 h-3 bg-black border-2 border-white rounded-full"></span>
                            )}
                          </div>
                          <div>
                            <h3 style={{ fontSize: '0.875rem', fontWeight: 700, color: 'var(--text-primary)', letterSpacing: '-0.02em', fontFamily: 'var(--font-heading)' }}>{p.name}</h3>
                            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.625rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginTop: '1px' }}>#{p._id.slice(-6).toUpperCase()}</p>
                          </div>
                        </div>
                      </td>
                      <td>
                        <span style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', fontWeight: 500, textTransform: 'capitalize' }}>{p.gender} · {p.age}y</span>
                      </td>
                      <td>
                        <span style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', fontWeight: 400, fontFamily: 'var(--font-mono)' }}>{p.contact || '—'}</span>
                      </td>
                      <td>
                        {p.allergies?.length > 0 ? (
                      ...
                      <td className="text-right">
                        <button 
                          className="btn-open-ehr"
                        >
                          Open EHR Record <ChevronRight size={13} />
                        </button>
                      </td>
                    </tr>
                  ))}
  ...
  const renderPatientDetailView = () => {
    ...
    return (
      <div className="flex-1 overflow-y-auto bg-[#f9f9f9] rounded-[24px] border border-slate-200/60 p-8 flex flex-col gap-8">
        ...
        <div className="breadcrumb-nav flex items-center gap-3 border-b border-slate-200/40 pb-4 shrink-0">
          <button 
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 border-b border-slate-200/40 pb-6">
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1 pt-1">
              ...
              <p className="font-mono text-[9px] text-slate-400 uppercase tracking-widest font-semibold mb-3">ID: #PT-{activePatient._id.slice(-6).toUpperCase()}</p>
              <div className="flex flex-wrap gap-x-8 gap-y-3">
                <div>
                  <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">Gender</p>
                  <p className="font-bold text-xs text-slate-700 capitalize">{activePatient.gender}</p>
                </div>
                <div>
                  <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">Age</p>
                ...
                <div>
                  <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">Allergies</p>
                  {activePatient.allergies?.length > 0 ? (
                     <p className="font-bold text-xs text-slate-800 uppercase flex items-center gap-1">
                      <AlertTriangle size={11} className="text-slate-500" /> {activePatient.allergies.join(', ')}
                    </p>
                  ) : (
          ...
          <div className="flex items-center gap-2 self-stretch md:self-auto justify-end shrink-0 animate-fade-in">
            ...
            >
              <Activity size={14} className="text-slate-500" /> Update Record
            </button>
            <button 
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'overview' && (
              <motion.div 
                ...
                className="ehr-overview-grid"
              >
                {/* Vitals Signs Grid - Modern Minimalist */}
                <div className="vitals-full-width">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
                    <h3 className="text-xs font-bold text-slate-800 font-heading">Current Physiological Metrics</h3>
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">As of last clinical consultation</span>
                  </div>
                  
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 vitals-grid">
                    {/* Heart Rate */}
                    <div className="minimalist-vitals-card bg-white border border-slate-200/60 rounded-xl p-4 flex flex-col justify-between shadow-sm">
                      <div className="flex justify-between items-center text-slate-400">
                        <span className="text-[10px] font-bold uppercase tracking-wide label">Heart Rate</span>
                        <Heart size={14} className="text-slate-400" />
                      </div>
                      <div className="mt-2.5">
                        <span className="text-2xl font-extrabold text-slate-900 font-mono value">{vitals.heartRate}</span>
                        <span className="text-[10px] text-slate-450 font-semibold ml-1 unit">bpm</span>
                      </div>
                      <div className="text-[9px] text-slate-400 font-semibold mt-1 normal-range">Normal: 60-100</div>
                    </div>

                    {/* Blood Pressure */}
                    <div className="minimalist-vitals-card bg-white border border-slate-200/60 rounded-xl p-4 flex flex-col justify-between shadow-sm">
                      <div className="flex justify-between items-center text-slate-400">
                        <span className="text-[10px] font-bold uppercase tracking-wide label">Blood Pressure</span>
                        <Activity size={14} className="text-slate-400" />
                      </div>
                      <div className="mt-2.5">
                        <span className="text-2xl font-extrabold text-slate-900 font-mono value">{vitals.bloodPressure}</span>
                        <span className="text-[10px] text-slate-450 font-semibold ml-1 unit">mmHg</span>
                      </div>
                      <div className="text-[9px] text-slate-400 font-semibold mt-1 normal-range">Normal: 120/80</div>
                    </div>

                    {/* Blood Glucose */}
                    <div className="minimalist-vitals-card bg-white border border-slate-200/60 rounded-xl p-4 flex flex-col justify-between shadow-sm">
                      <div className="flex justify-between items-center text-slate-400">
                        <span className="text-[10px] font-bold uppercase tracking-wide label">Blood Glucose</span>
                        <Droplet size={14} className="text-slate-400" />
                      </div>
                      <div className="mt-2.5">
                        <span className="text-2xl font-extrabold text-slate-900 font-mono value">{vitals.bloodGlucose}</span>
                        <span className="text-[10px] text-slate-450 font-semibold ml-1 unit">mg/dL</span>
                      </div>
                      <div className="text-[9px] text-slate-400 font-semibold mt-1 normal-range">Normal: 70-140</div>
                    </div>

                    {/* SpO2 */}
                    <div className="minimalist-vitals-card bg-white border border-slate-200/60 rounded-xl p-4 flex flex-col justify-between shadow-sm">
                      <div className="flex justify-between items-center text-slate-400">
                        <span className="text-[10px] font-bold uppercase tracking-wide label">SpO2</span>
                        <Wind size={14} className="text-slate-400" />
                      </div>
                      <div className="mt-2.5">
                        <span className="text-2xl font-extrabold text-slate-900 font-mono value">{vitals.spo2}</span>
                        <span className="text-[10px] text-slate-450 font-semibold ml-1 unit">%</span>
                      </div>
                      <div className="text-[9px] text-slate-400 font-semibold mt-1 normal-range">Normal: 95-100</div>
                    </div>

                    {/* Temperature */}
                    <div className="minimalist-vitals-card bg-white border border-slate-200/60 rounded-xl p-4 flex flex-col justify-between shadow-sm">
                      <div className="flex justify-between items-center text-slate-400">
                        <span className="text-[10px] font-bold uppercase tracking-wide label">Temperature</span>
                        <Thermometer size={14} className="text-slate-400" />
                      </div>
                      <div className="mt-2.5">
                        <span className="text-2xl font-extrabold text-slate-900 font-mono value">{vitals.temperature}</span>
                        <span className="text-[10px] text-slate-450 font-semibold ml-1 unit">°F</span>
                      </div>
                      <div className="text-[9px] text-slate-400 font-semibold mt-1 normal-range">Normal: 97-99</div>
                    </div>

                    {/* Respiratory Rate */}
                    <div className="minimalist-vitals-card bg-white border border-slate-200/60 rounded-xl p-4 flex flex-col justify-between shadow-sm">
                      <div className="flex justify-between items-center text-slate-400">
                        <span className="text-[10px] font-bold uppercase tracking-wide label">Respirations</span>
                        <Activity size={14} className="text-slate-400" />
                      </div>
                      <div className="mt-2.5">
                        <span className="text-2xl font-extrabold text-slate-900 font-mono value">{vitals.respiratoryRate}</span>
                        <span className="text-[10px] text-slate-450 font-semibold ml-1 unit">/min</span>
                      </div>
                      <div className="text-[9px] text-slate-400 font-semibold mt-1 normal-range">Normal: 12-20</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  {/* Clean Blood Group Card */}
                  <div className="bg-white border border-slate-200/65 rounded-2xl p-5 shadow-sm flex items-center justify-between">
                    <div>
                      <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block mb-0.5">Blood Profile</span>
                      <h3 className="text-xs font-bold text-slate-800">Verified Blood Group</h3>
                    </div>
                    <div className="flex items-center gap-3 bg-slate-50 px-4 py-2 rounded-xl border border-slate-200 shadow-sm shrink-0">
                      <Droplet className="text-slate-400" size={16} />
                      <span className="text-xl font-extrabold tracking-tight text-slate-900 font-mono">{activePatient.bloodGroup || 'N/A'}</span>
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-slate-200/50 rounded-2xl p-6 shadow-sm space-y-5">
                    <div className="border-b border-slate-100 pb-3">
                      <h3 className="text-xs font-bold text-slate-850">Residential & Base Records</h3>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4 pt-1">
                        <div>
                          ...
                          <p className="text-xs font-bold text-slate-850">{activePatient.contact || 'N/A'}</p>
                        </div>
                        <div>
                          <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block mb-0.5">Age Bracket</span>
                          <p className="text-xs font-bold text-slate-850">{activePatient.age} Years ({activePatient.gender})</p>
                        </div>
                      </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white border border-slate-200/50 rounded-2xl p-6 shadow-sm">
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="text-xs font-bold text-slate-800 font-heading">Recent Diagnostic Scans</h3>
                      <button className="text-slate-600 text-xs font-bold hover:underline cursor-pointer border-none bg-transparent">View All</button>
                    </div>
                  ...
                  <div className="bg-white border border-slate-200/50 rounded-2xl p-6 shadow-sm flex flex-col gap-4">
                    <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <h3 className="text-xs font-bold text-slate-800">MedFlow Clinical AI</h3>
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <form onSubmit={handleAddCustomSymptom} className="flex gap-2">
                        <input 
                          ...
                          className="flex-1 bg-slate-50 border border-slate-200 rounded-full px-4 py-2 text-xs font-semibold outline-none focus:border-slate-400"
                        />
                    ...
                    <div className="ai-diagnosis-result-area">
                      {aiDiagnosis ? (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 pt-3 border-t border-slate-100">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {aiDiagnosis.conditions?.map((c, i) => (
                              <div key={i} className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 flex flex-col gap-1.5">
                                <div className="flex justify-between items-center text-xs font-bold text-slate-700">
                                  <span>{c.name}</span>
                                  <span className="font-mono text-[9px] text-slate-400">{c.probability}</span>
                                </div>
                          ...
                          <div className="grid grid-cols-2 gap-4 mt-2 pt-3 border-t border-slate-100">
                            <div className="flex flex-col gap-1">
                              ...
                              <span className="inline-flex items-center gap-1 self-start text-[9px] font-bold uppercase px-2.5 py-0.5 rounded-full bg-slate-100 border border-slate-250 text-slate-705">
                            ...
                            <div className="flex flex-col gap-1">
                              ...
                              <span className="text-[10px] text-slate-700 font-bold bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1">
                        ...
                        <div className="border border-dashed border-slate-200 rounded-xl p-6 text-center">
                          <Brain size={20} className="text-slate-350 mx-auto mb-1.5 opacity-60" />
            ...
            {activeTab === 'appointments' && (
              <motion.div 
                ...
                className="space-y-4"
              >
                <h3 className="text-xs font-bold text-slate-800">Visit Timeline History</h3>
                {activeAppointments.length === 0 ? (
                  <div className="text-center p-12 bg-white rounded-2xl border border-slate-150 shadow-sm">
                    <Clock size={24} className="text-slate-350 mx-auto mb-2 opacity-50" />
                    <p className="text-xs text-slate-450 font-semibold">No visits recorded in database metrics.</p>
                  </div>
                ) : (
                  <div className="flex flex-col gap-3">
                    {activeAppointments.map(app => (
                      <div key={app._id} className="bg-white border border-slate-200 hover:border-slate-300 transition-colors p-4 rounded-xl flex items-center justify-between shadow-sm">
                        <div className="flex items-center gap-4">
                          <div className="bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-lg text-center shrink-0">
                            ...
                            <span className="block text-sm font-extrabold text-slate-850 font-mono">
                              {new Date(app.date).toLocaleDateString(undefined, { day: 'numeric' })}
                            </span>
                          </div>
                          <div>
                            <h4 className="font-bold text-xs text-slate-805 leading-tight">{app.notes || 'Clinical Consultation'}</h4>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-[9px] uppercase px-2.5 py-0.5 rounded-full font-bold border bg-slate-50 border-slate-200 text-slate-600">
                            {app.status}
                          </span>
                        </div>
                      </div>
            ...
            {activeTab === 'history' && (
              ...
              >
                <div className="flex justify-between items-center">
                  <h3 className="text-xs font-bold text-slate-805">EHR Digital Prescriptions</h3>
                  <button 
                    onClick={() => setIsPrescriptionModalOpen(true)}
                    className="px-3.5 py-1.5 rounded-full bg-black text-[#c8f17a] hover:bg-slate-900 text-[10px] font-extrabold flex items-center gap-1.5 transition-colors cursor-pointer shadow-sm border-none uppercase tracking-wide"
                  >
                ...
                {activePrescriptions.length === 0 ? (
                  <div className="text-center p-12 bg-white rounded-2xl border border-slate-150 shadow-sm">
                    <FileText size={24} className="text-slate-350 mx-auto mb-2 opacity-50" />
                    <p className="text-xs text-slate-450 font-semibold">No active electronic health prescriptions found.</p>
                  </div>
                ) : (
                  <div className="timeline-stack flex flex-col gap-6 relative pl-6 border-l border-slate-200 mt-2">
                    {activePrescriptions.map(pr => (
                      <div key={pr._id} className="timeline-item relative">
                        <div className="absolute -left-[31px] top-2.5 w-2 h-2 rounded-full bg-black border-2 border-white ring-4 ring-slate-100"></div>
                        
                        <div className="bg-white p-5 border border-slate-200 rounded-2xl shadow-sm hover:border-slate-350 transition-colors flex flex-col gap-3">
                          ...
                          <div className="flex flex-col gap-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {pr.medicines?.map((med, mIdx) => (
                                <span key={mIdx} className="text-xs font-semibold bg-slate-50 border border-slate-200 px-2.5 py-1 rounded-lg text-slate-700">
  ...
  return (
    <div className="patient-module h-full overflow-hidden">
      <AnimatePresence mode="wait">
        {!activePatient ? (
          <motion.div
            key="list"
          ...
          >
            ...
            <form onSubmit={handleAddPrescriptionSubmit} className="flex flex-col gap-4">
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <button 
                    ...
                    className="text-xs text-slate-600 font-bold hover:underline cursor-pointer border-none bg-transparent flex items-center gap-1"
                  >
                      ...
                      <div className="grid grid-cols-4 gap-2 flex-1">
                        <div>
                          ...
                          <input 
                            ...
                            className="w-full bg-white border border-slate-200 rounded-lg p-1.5 text-xs font-semibold outline-none focus:border-slate-400"
                            value={med.name} 
              ...
              <div className="input-group">
                ...
                <textarea 
                  rows="2"
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-xs font-semibold outline-none focus:border-slate-400" 
